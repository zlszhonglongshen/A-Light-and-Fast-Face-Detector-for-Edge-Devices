...
class Predict(object):
    ...
    def __load_model(self):
        # load onnx
        print('----> load onnx file: %s' % self.onnx_file_path)
        if not os.path.exists(self.onnx_file_path):
            print('The onnx file does not exist!!!!')
            sys.exit(1)
        net_symbol, arg, aux = self.mxnet.contrib.onnx.import_model(self.onnx_file_path)

        data_name = 'data'
        data_name_shape = (data_name, (1, 3, self.input_height, self.input_width))
        self.module = self.mxnet.module.Module(symbol=net_symbol,
        ...
        self.aux_name_arrays = dict()
        for k, v in arg.items():
            self.arg_name_arrays.update({k: v.as_in_context(self.ctx)})
        for k, v in aux.items():
            self.aux_name_arrays.update({k: v.as_in_context(self.ctx)})
...
def run_prediction_folder():
    ...
    for file_name in file_name_list:
        ...
        bboxes = my_predictor.predict(im, resize_scale=1, score_threshold=0.4, top_k=10000, NMS_threshold=0.2, NMS_flag=True, skip_scale_branch_list=[])
        for bbox in bboxes:
